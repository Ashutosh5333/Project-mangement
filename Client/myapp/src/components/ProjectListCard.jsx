...
const ProjectListCard = () => {
  ...
  async function getdata() {
    try {
      const res = await fetch(
        `https://techback.onrender.com/project?sortBy=${sortBy}&page=${current}&limit=${page}`
      );
  ...
  const handlecancel = async (id) => {
    try {
      const res = await axios.patch(
        `https://techback.onrender.com/statuscancel/${id}`
      );
      getdata();
    } catch (err) {
      console.log(err);
  ...
  const handleClose = async (id) => {
    try {
      const res = await axios.patch(
        `https://techback.onrender.com/statusclose/${id}`
      );
      getdata();
    } catch (err) {
      console.log(err);
  ...
  const handleRunning = async (id) => {
    try {
      const res = await axios.patch(
        `https://techback.onrender.com/statusrun/${id}`
      );
      getdata();
    } catch (err) {
      console.log(err);
                  ...
                  {Projectdata.length > 0 ? (
                    Projectdata.filter((value) => {
                      if (inputdata == "") {
                        ...
                        value.Projecttheme.toLowerCase().includes(
